...
export default function AssignmentManager() {
  ...
  const handleSubjectChange = (e) => {
    ...
    if (subject) {
      const courseId = String(subject.course?._id || subject.course || "");
      setForm((f) => ({
        ...f,
        subject: subjectId,
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editItem) {
        // Plain JSON update
        const { data } = await api.put(`/assignments/${editItem._id}`, payload);
        savedId = editItem._id;
        // Upload file separately if selected
        if (selectedFile) {
          await uploadFileToAssignment(savedId, selectedFile);
        }
      } else {
        // Create — send as multipart if file selected
        if (selectedFile) {
  ...
  const uploadFileToAssignment = async (assignmentId, file) => {
    const fd = new FormData();
    fd.append("attachment", file);
    await api.post(`/assignments/${assignmentId}/upload`, fd, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  };

  /* ── standalone upload from table row ── */
  const handleRowUpload = async (assignmentId, file) => {
    if (!file) return;
    setUploadingId(assignmentId);
    try {
      await uploadFileToAssignment(assignmentId, file);
      dispatch(fetchAssignments({}));
    } catch (err) {
  ...
  /* ── remove attachment ── */
  const handleRemoveAttachment = async (assignmentId) => {
    if (!window.confirm("Remove the attachment from this assignment?")) return;
    setRemovingId(assignmentId);
    try {
      await api.delete(`/assignments/${assignmentId}/upload`);
      dispatch(fetchAssignments({}));
    } catch (err) {
  ...
  const handleDelete = async (id, title) => {
    if (
        !window.confirm(`Delete "${title}"? All submissions will be deleted too.`)
    )
      return;
    setDeletingId(id);
    try {
  ...
  const subjectPlaceholder = useCallback(() => {
    ...
    if (allSubjects.length === 0) return "No subjects assigned to you";
    if (form.course && filteredSubjects.length === 0)
      return "No subjects for this course";
          ...
          <div className="page-body">
            ...
            >
              {[
                ...
                {
                  label: "Published",
                  val: assignments.filter((a) => a.isPublished).length,
                  color: "#059669",
                },
                {
                  label: "Active",
                  val: assignments.filter((a) => !isPast(a.dueDate)).length,
                  color: "#D97706",
                },
                {
                  label: "Past due",
                ...
                {
                  label: "With file",
                  val: assignments.filter((a) => a.attachmentUrl).length,
                  color: "#7C3AED",
                },
              ].map((s) => (
                  <div
                  ...
                  <table className="data-table">
                    ...
                    {assignments.map((a) => (
                        ...
                        >
                          ...
                          {isPast(a.dueDate) && (
                              <span
                                  style={{ marginLeft: 3, fontSize: "0.65rem" }}
                              >
                              (closed)
                            </span>
                          )}
                            ...
                            {a.attachmentUrl ? (
                                ...
                                >
                                  <a
                                      ...
                                      title={getFileName(a.attachmentUrl)}
                                  >
                                    {getFileIcon(a.attachmentUrl)}
                                  </a>
                                  ...
                                  <button
                                      ...
                                      onClick={() => handleRemoveAttachment(a._id)}
                                ...
                                >
                                  <input
                                      ...
                                      style={{ display: "none" }}
                                      onChange={(e) =>
                                          handleRowUpload(a._id, e.target.files[0])
                                      }
                          ...
                          <td>
                        <span
                            className={`badge ${a.isPublished ? "badge-success" : "badge-error"}`}
                        >
                ...
                <form className="modal__body" onSubmit={handleSave}>
                  ...
                  <div className="form-group">
                    <label className="form-label">
                      ...
                      >
                    — pick a subject and course auto-fills
                  </span>
                    </label>
                    ...
                    {!subjectsLoading && allSubjects.length === 0 && (
                        ...
                        >
                          ⚠ No subjects assigned to you yet. Ask your admin.
                        </div>
                    )}
                  ...
                  <div className="form-group">
                    <label className="form-label">
                      ...
                      >
                    PDF, Word, PPT, images, ZIP — max 20 MB
                  </span>
                    </label>

                    {/* Existing attachment when editing */}
                    {editItem?.attachmentUrl && !selectedFile && (
                        <div
                            style={{
                              display: "flex",
                              alignItems: "center",
                    ...
                    <span style={{ fontSize: "1.25rem" }}>
                      {getFileIcon(editItem.attachmentUrl)}
                    </span>
                          <div style={{ flex: 1 }}>
                            <div
                                style={{
                            ...
                            <a
                                href={editItem.attachmentUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                style={{ fontSize: "0.75rem", color: "#059669" }}
                            >
                              {getFileName(editItem.attachmentUrl)}
                            </a>
                          </div>
                        ...
                        {FILE_ICONS[
                          ...
                          <div className="file-drop-zone__empty">
                            ...
                            <div>
                              ...
                              >
                                PDF, DOC, PPT, images, ZIP up to 20 MB
                              </div>
                            </div>
                          </div>
                      )}
