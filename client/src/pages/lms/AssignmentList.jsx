...
export default function AssignmentList() {
  ...
  const { myEnrollments, loading: enrollLoading } = useSelector(
      (s) => s.subjectEnrollments,
  );
  ...
  /* ── load assignments — backend resolves subject+section enrollment itself ── */
  useEffect(() => {
    const active = myEnrollments.filter((e) => e.status === "active");
    if (active.length === 0) { setAssignments([]); return; }

    const load = async () => {
      setAssLoading(true);
      try {
        const { data } = await api.get("/assignments/student/mine");
  ...
  const enrolledSubjects = myEnrollments
      .filter((e) => e.status === "active" && e.subject)
      .map((e) => ({
        _id: e.subject?._id || e.subject,
        name: e.subject?.name || e.subject?.code || "Subject",
      }))
      .filter(
          (s, i, arr) =>
  ...
  const filtered = assignments.filter((a) => {
    if (
        subjectFilter !== "all" &&
        String(a.subject?._id || a.subject) !== String(subjectFilter)
    )
      return false;
          ...
          {assignments.length > 0 && (
              <div className="assignment-filter-bar">
                ...
                {enrolledSubjects.length > 1 && (
                    <div className="assignment-filter-group">
                      <button
                          className={`assignment-filter-btn ${subjectFilter === "all" ? "active" : ""}`}
                      ...
                      {enrolledSubjects.map((s) => (
                          <button
                              key={s._id}
                              className={`assignment-filter-btn ${subjectFilter === String(s._id) ? "active" : ""}`}
                              onClick={() => setSubjectFilter(String(s._id))}
                          >
                            {s.name}
                          </button>
                      ))}
            ...
            <div className="assignment-list-pane">
              ...
              0 ? (
                  <div className="empty-state">
                    ...
                    <p>Not enrolled in any subjects.</p>
                  </div>
              ) : assignments.length === 0 ? (
                  <div className="empty-state">
                    <div className="empty-state__icon">📝</div>
