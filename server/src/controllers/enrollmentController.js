    ...
    try {
        ...
        if (existing) {
            ...
            const populated = await existing.populate([
                { path: 'course',   select: 'title code department duration maxStudents status' },
        ...
        } catch (_) { /* never block enrollment for notification failure */ }
...
exports.dropCourse = async (req, res) => {
    try {
        ...
        if (
            String(enrollment.student) !== String(req.user._id) &&
            req.user.role !== 'admin'
        ) {
...
// GET /api/enrollments/course/:courseId — admin/teacher: all students in a course
